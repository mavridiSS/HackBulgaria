...
class WhoFollowsYouBack:
    ...
    def get_network_for(user):
        ...
        config = WhoFollowsYouBack.load('client.json')
        ...
        while has_followers:
            followers = requests.get('https://api.github.com/users/'
                                     ...
                                     + secret)
            if followers.json():
                network['followers'] += WhoFollowsYouBack.usernames_from(followers)
            else:
                has_followers = False
            cur_followers_page += 1
        ...
        while has_following:
            following = requests.get('https://api.github.com/users/'
                                     ...
                                     + secret)
            if following.json():
                network['following'] += WhoFollowsYouBack.usernames_from(following)
            else:
                has_following = False
    ...
    def build_network(self, user, level):
        ...
        while len(queue) > 0:
            ...
            if curr_lvl + 1 > level:
                break
            network = WhoFollowsYouBack.get_network_for(curr_node)
...
a = WhoFollowsYouBack("mavridiSS", 2)
print(a.who_follows_you_back())
